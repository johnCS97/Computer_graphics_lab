...
class SmoothL:
    ...
    def create_cells(self, new_cells, age):
        for i in range(new_cells):
            ...
            self.game_field[x-self.outer:x + self.outer, y -
                            self.outer:y + self.outer, z-self.outer:z + self.outer] = age
...
if __name__ == '__main__':
    ...
    def animate(*args):
        plt.cla()
        ax.axis('off')
        ax.voxels(sm.next(), facecolor="#4d4d4d",
                  edgecolors="#4d4d4d", shade=True)
        return ax,
